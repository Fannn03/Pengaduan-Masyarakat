...
<div class="container border mx-auto rounded-md p-5 text-white">
    <h1 class="text-xl border-b p-3 block">Edit Laporan {{ $laporan->judul_laporan }}</h1>
    <div class="flex flex-row">
        <form action="{{ url('/dashboard/edit-laporan/' . $laporan->slug) }}" method="post" class="w-1/2 p-5 space-y-5" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex flex-col space-y-3">
                ...
                <textarea name="isi_laporan"
                    ...
                    rows="3">{{ $laporan->isi_laporan }}</textarea>
                @error('isi_laporan')
                <p class="text-red-500 text-xs">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex flex-row space-x-3">
                <input type="submit" value="Edit Laporan"
        ...
        <div class="flex flex-1 justify-center">
            <img src="{{ asset('storage/images/laporan/' . $laporan->foto_laporan) }}" id='blah' class="rounded-md h-2/3 my-auto" alt="">
        </div>
    </div>
</div>
@endsection
