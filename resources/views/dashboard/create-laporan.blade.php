...
<div class="container border mx-auto flex flex-col rounded-md p-5 space-y-3 font-mono">
    ...
    <form action="" method="POST" enctype="multipart/form-data" class="text-gray-100 flex flex-col space-y-3 px-3">
        @csrf
        <div class="flex flex-row">
            ...
            <div class="flex flex-col w-1/2 space-y-3">
                ...
                <div class="flex flex-col space-y-3">
                    ...
                    <label class="bg-white rounded w-2/3 text-black flex justify-between group cursor-pointer @error('foto_laporan') border-red-500 border-2 @enderror hover:bg-purple-500 transition duration-300">
                ...
                <div class="flex flex-col space-y-3">
                    ...
                    <textarea name="isi_laporan" class="text-black rounded-md p-3 w-full @error('isi_laporan') border-red-500 border-2 @enderror" rows="3">{{ old('isi_laporan') }}</textarea>
                    @error('isi_laporan')
                    <p class="text-red-500 text-xs">{{ $message }}</p>
                    @enderror
                </div>
